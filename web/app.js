import { explanationScope } from "./explanation-scope.mjs";
...
import { beginnerLines } from "./home-copy.mjs";
...
if (languageToggle) {
  languageToggle.textContent = isEnglish ? "한국어" : "EN";
  languageToggle.setAttribute("aria-label", isEnglish ? "한국어로 전환" : "Switch to English");
  languageToggle.addEventListener("click", () => {
    const params = new URLSearchParams(location.search);
    if (isEnglish) params.delete("lang"); else params.set("lang", "en");
    const query = params.toString();
    location.assign(`${location.pathname}${query ? `?${query}` : ""}${location.hash || "#summary"}`);
  });
}

if (isEnglish) {
  const navLabels = { summary: "Project", global: "Global data", risks: "Review queue", limitations: "Validation & limits" };
  document.querySelectorAll("nav a[data-route]").forEach(link => { link.textContent = navLabels[link.dataset.route] || link.textContent; });
}
...
function validatePhase1(phase1) {
  ...
  const paired = phase1.paired_model_comparison;
  const pairedValues = [paired?.paired_repeats, paired?.mean_difference, paired?.challenger_repeat_wins, paired?.two_sided_exact_sign_flip_p];
  if (!pairedValues.every(Number.isFinite)) throw new Error("Phase 1 paired 비교 데이터 계약이 올바르지 않습니다.");
  if (paired.reference_candidate !== "l1_logistic_c_0.01" || paired.challenger_candidate !== "random_forest_depth_none_leaf_8") throw new Error("Phase 1 paired 비교 후보가 올바르지 않습니다.");
  if (paired.pairing_unit !== "repeat_mean_of_shared_5_folds") throw new Error("Phase 1 paired 비교 단위가 올바르지 않습니다.");
  if (!Number.isInteger(paired.paired_repeats) || paired.paired_repeats <= 0) throw new Error("Phase 1 paired repeat 수가 올바르지 않습니다.");
  if (!Number.isInteger(paired.challenger_repeat_wins) || paired.challenger_repeat_wins < 0 || paired.challenger_repeat_wins > paired.paired_repeats) throw new Error("Phase 1 paired 승리 수가 올바르지 않습니다.");
  if (paired.two_sided_exact_sign_flip_p < 0 || paired.two_sided_exact_sign_flip_p > 1) throw new Error("Phase 1 paired p-value가 올바르지 않습니다.");
...
function validateGlobalE2E(summary) {
  ...
  const enedis = summary.sources?.enedis_france_2024;
  if (!enedis || enedis.status !== "sdt_contract_validated" || enedis.source_type !== "estimated" || enedis.frictionless_valid !== true || enedis.input_rows !== 17568 || enedis.missing_energy_rows !== 207 || enedis.data_quality_warning !== true || !/^[a-f0-9]{64}$/.test(enedis.input_sha256)) throw new Error("Enedis E2E 검증 상태가 올바르지 않습니다.");
...
function candidateRegistry(registry) {
  return `<details class="candidate-registry">
    <summary><span>EXPANSION CANDIDATES</span><strong>${isEnglish ? "View country and robotics data candidates" : "후속 국가·로봇 데이터 후보 보기"}</strong><small>${isEnglish ? `${registry.candidates.length} countries · research list, not connected sources` : `${registry.candidates.length}개 국가 · 연결 완료가 아닌 조사 목록`}</small></summary>
    <div class="candidate-intro">${isEnglish ? "Candidates advance to implementation only after checks for official provenance, access, licensing, units, time zones and independent research value." : "후보는 공식 출처·접근성·라이선스·단위·시간대·독립적 연구가치를 통과한 뒤에만 구현 단계로 승격합니다."}</div>
    ...
    <p class="candidate-boundary">${isEnglish ? "Planning registry only. Inclusion does not mean the source is connected, validated or evidence of semiconductor-model performance." : esc(registry.claim_boundary)}</p>
  </details>`;
}
...
function waferVisual(featCount) {
  return `<figure class="hero-visual"><img src="/assets/fabguard-dusk-hero-v3.jpg" alt="${isEnglish ? "FabGuard concept image of an engineer reviewing manufacturing data near a semiconductor fab" : "노을에서 야간으로 이어지는 대형 반도체 팹과 엔지니어의 데이터 기반 위험 검토를 표현한 FabGuard 콘셉트 이미지"}"><div class="visual-shade"></div><figcaption>${isEnglish ? "Original concept artwork · not a real fab or deployment photo." : "독자 제작 콘셉트 이미지 · 실제 공장 또는 현장 배포 사진이 아닙니다."}</figcaption><div class="visual-stat"><span>${isEnglish ? "INPUT" : "분석 입력"}</span><strong>${featCount}</strong><small>${isEnglish ? "anonymous measurements" : "익명 측정변수"}</small></div><div class="visual-status"><i></i> HUMAN REVIEW REQUIRED</div></figure>`;
}

function beginnerEntry() {
  const lines = beginnerLines(language);
  return `<section class="absolute-beginner" aria-labelledby="beginner-title"><div><p class="kicker">${isEnglish ? "START HERE" : "처음 오셨다면"}</p><h2 id="beginner-title">${isEnglish ? "FabGuard in three lines" : "FabGuard를 딱 세 줄로"}</h2></div><ol>${lines.map(line => `<li>${line}</li>`).join("")}</ol><div class="beginner-actions"><a class="button" href="#answer-overview">${isEnglish ? "Learn more" : "더 자세히 보기"} <span>→</span></a><a class="text-link" href="#result">${isEnglish ? "See the result only" : "그냥 결과만 보기"} <span>→</span></a></div></section>`;
}
...
function summaryView() {
  ...
  app.innerHTML = `
    <section class="hero"><div class="hero-copy"><div class="status-chip"><i></i> ${isEnglish ? "PUBLIC SEMICONDUCTOR DATA · OFFLINE DEMO" : "공개 반도체 데이터 · 오프라인 데모"}</div><p class="kicker">FABGUARD AI</p><h1>${isEnglish ? "When you cannot inspect everything,<br><span>inspect what matters first.</span>" : "모두 볼 수 없다면,<br><span>위험한 것부터.</span>"}</h1><p class="hero-ko">${isEnglish ? "Rank semiconductor production records and<br><strong>suggest where an engineer should start.</strong>" : "반도체 생산 기록의 위험도를 정렬해<br><strong>엔지니어의 첫 점검 대상을 제안합니다.</strong>"}</p><p class="lead">${isEnglish ? "FabGuard does not make pass/fail decisions or control a process. It supports a constrained question: when inspection capacity is limited, which records should an engineer review first? Failed classification performance is disclosed and reframed as decision support rather than hidden." : "AI가 불량을 확정하거나 공정을 제어하지 않습니다. 제한된 점검 시간을 어디에 먼저 쓸지 보여주고, 최종 판단은 엔지니어가 합니다. 실패한 성능을 숨기지 않고, 제한된 자원에서 쓸 수 있는 판단 근거로 다시 설계했습니다."}</p>${renderHeroEvidence(state.summary, language)}<div class="hero-actions"><a class="button" href="#risks">${isEnglish ? "Open the review queue" : "점검 목록 직접 보기"} <span>→</span></a><a class="text-link" href="#process">${isEnglish ? "See the decision and validation process" : "판단과 검증 과정 보기"}</a></div></div>${waferVisual(ds.measurement_features)}</section>
    ${beginnerEntry()}
    <section class="answer-strip" id="answer-overview" aria-label="${isEnglish ? "FabGuard summary" : "FabGuard 핵심 요약"}">${isEnglish ? `<article><span>PROBLEM</span><strong>Not every production record<br>can be inspected at once</strong></article><article class="active"><span>FABGUARD</span><strong>Ranks records for review<br>and shows evidence limits</strong></article><article><span>HUMAN ROLE</span><strong>Engineers inspect<br>and decide the action</strong></article><article class="boundary"><span>CURRENT BOUNDARY</span><strong>Public-data experiment<br>not field validated</strong></article>` : `<article><span>문제</span><strong>모든 생산 건을<br>동시에 볼 수 없음</strong></article><article class="active"><span>FabGuard</span><strong>위험도 순으로 추천하고<br>근거의 한계까지 표시</strong></article><article><span>사람의 역할</span><strong>엔지니어가 확인하고<br>최종 조치를 결정</strong></article><article class="boundary"><span>현재 경계</span><strong>공개데이터 실험<br>현장 효과는 미검증</strong></article>`}</section>
    <section class="global-section" id="global" aria-labelledby="global-title">
      <div class="global-heading">
        <div><p class="kicker">GLOBAL DATA NETWORK / SYSTEM DEMO</p><h2 id="global-title">${isEnglish ? "Different countries.<br><span>One audit contract.</span>" : "서로 다른 국가의 데이터를<br><span>같은 감사 규칙으로.</span>"}</h2></div>
        <div class="global-summary"><strong>4</strong><span>${isEnglish ? "DATA ROLES" : "데이터 유형"}</span><p>${isEnglish ? "Observed, estimated, reference and synthetic data stay distinct. Provenance and validation status matter more than country count." : "관측 · 추정 · 기준 · 합성을 구분하며, 국가 수보다 출처와 검증 상태를 우선합니다."}</p><div class="data-role-key" aria-label="${isEnglish ? "Data-role colour key" : "데이터 유형 색상 구분"}">${dataRole("observed", isEnglish ? "Observed" : "관측")}${dataRole("estimated", isEnglish ? "Estimated" : "추정")}${dataRole("reference", isEnglish ? "Reference" : "기준")}${dataRole("synthetic", isEnglish ? "Synthetic" : "합성")}</div></div>
      </div>
      ...
      <div class="country-grid" aria-label="국가별 데이터 검증 상태">
        <article class="country-card verified"><div class="country-top">${flagImage("us", isEnglish ? "United States" : "미국")}<b>${isEnglish ? "UNITED STATES" : "미국"}</b><em>${isEnglish ? "VERIFIED" : "검증됨"}</em></div><h3>UCI SECOM</h3><p>${isEnglish ? "Canonical data for semiconductor process review-ranking research" : "반도체 공정 위험순위 연구의 정본 데이터"}</p><dl><div><dt>${isEnglish ? "ROLE" : "역할"}</dt><dd>${isEnglish ? "Manufacturing evidence" : "제조 연구 근거"}</dd></div><div><dt>${isEnglish ? "STATUS" : "현황"}</dt><dd>${isEnglish ? "V1 complete" : "V1 완료"}</dd></div></dl></article>
        <article class="country-card verified"><div class="country-top">${flagImage("au", isEnglish ? "Australia" : "호주")}<b>${isEnglish ? "AUSTRALIA" : "호주"}</b><em>${isEnglish ? "VERIFIED" : "검증됨"}</em></div><h3>DKASC</h3><p>${isEnglish ? "Observed 2025 time series from Alice Springs" : "Alice Springs 2025 실제 관측 시계열"}</p><dl><div><dt>${isEnglish ? "ROLE" : "역할"}</dt><dd>${dataRole("observed", isEnglish ? "Observed" : "관측")}</dd></div><div><dt>${isEnglish ? "STATUS" : "현황"}</dt><dd>${isEnglish ? "E2E contract validated" : "E2E 계약 검증"}</dd></div></dl></article>
        <article class="country-card verified"><div class="country-top">${flagImage("gb", isEnglish ? "Great Britain" : "영국")}<b>${isEnglish ? "GREAT BRITAIN" : "영국"}</b><em>${isEnglish ? "VERIFIED" : "검증됨"}</em></div><h3>PV_Live</h3><p>${isEnglish ? "Estimated national solar generation for Great Britain" : "GB 국가 단위 태양광 발전 추정값"}</p><dl><div><dt>${isEnglish ? "ROLE" : "역할"}</dt><dd>${dataRole("estimated", isEnglish ? "Estimated" : "추정")}</dd></div><div><dt>${isEnglish ? "STATUS" : "현황"}</dt><dd>${pvlive.input_rows.toLocaleString()}${isEnglish ? " intervals · E2E validated" : "개 간격 · E2E 검증"}</dd></div></dl></article>
        <article class="country-card verified"><div class="country-top">${flagImage("eu", isEnglish ? "European Union" : "유럽연합")}<b>${isEnglish ? "EUROPEAN UNION" : "유럽연합"}</b><em>${isEnglish ? "VERIFIED" : "검증됨"}</em></div><h3>JRC PVGIS</h3><p>${isEnglish ? "Reference solar time series based on Brussels weather and models" : "Brussels 기상·모델 기반 태양광 기준 시계열"}</p><dl><div><dt>${isEnglish ? "ROLE" : "역할"}</dt><dd>${dataRole("reference", isEnglish ? "Reference" : "기준")}</dd></div><div><dt>${isEnglish ? "STATUS" : "현황"}</dt><dd>${pvgis.input_rows.toLocaleString()}${isEnglish ? " hours · E2E validated" : "시간 · E2E 검증"}</dd></div></dl></article>
        <article class="country-card next"><div class="country-top">${flagImage("fr", isEnglish ? "France" : "프랑스")}<b>${isEnglish ? "FRANCE" : "프랑스"}</b><em>${isEnglish ? "E2E · QUALITY WARNING" : "E2E · 품질 경고"}</em></div><h3>RTE · Enedis · Météo-France</h3><p>${isEnglish ? "National generation revisions · distribution solar · observed Paris weather" : "국가 발전량 수정 이력 · 배전망 태양광 · 파리 관측 기상"}</p><dl><div><dt>${isEnglish ? "ROLE" : "역할"}</dt><dd><span class="role-pair">${dataRole("estimated", isEnglish ? "Grid estimate" : "전력망 추정")}${dataRole("observed", isEnglish ? "Weather observed" : "기상 관측")}</span></dd></div><div><dt>${isEnglish ? "STATUS" : "현황"}</dt><dd>${isEnglish ? `Enedis ${enedis.input_rows.toLocaleString()} intervals · Météo-France ${meteo.input_rows.toLocaleString()} hours verified · RTE 17,568 intervals verified · quality warning retained` : `Enedis ${enedis.input_rows.toLocaleString()}개 간격 · Météo-France ${meteo.input_rows.toLocaleString()}시간 검증 · RTE 17,568개 간격 검증 · 품질 경고 유지`}</dd></div></dl></article>
      </div>
    ...
    <section class="story-section" id="result"><div class="section-intro"><p class="kicker">${isEnglish ? "CURRENT RESULT" : "현재 결과"}</p><h2>${isEnglish ? `Reviewing ${ten.inspection_count} records first<br>captured ${ten.captured_fail} failures.` : `${ten.inspection_count}건을 먼저 봤을 때<br>불량 ${ten.captured_fail}건을 찾았습니다.`}</h2><p>${isEnglish ? `This result reviews the highest-ranked 10% instead of inspecting the whole temporal holdout. ${ten.total_fail - ten.captured_fail} of ${ten.total_fail} failures remained outside that review budget. The result is provisional because the holdout was exposed during an engineering smoke run; independent manufacturing data are still required.` : `전체 검증구간을 무작정 확인하는 대신 AI 위험도가 높은 상위 10%를 먼저 살펴본 결과입니다. 전체 불량 ${ten.total_fail}건 중 ${ten.total_fail - ten.captured_fail}건은 이 범위 밖에 남았습니다. 홀드아웃 사전 노출이 있는 잠정 결과이며 독립 제조 데이터 검증이 필요합니다.`}</p></div><div class="command-panel"><div class="panel-head"><span>${isEnglish ? "VALIDATION RESULT" : "검증 결과 요약"}</span><span class="live"><i></i> ${isEnglish ? "PROVISIONAL" : "잠정 결과"}</span></div><div class="metrics-grid"><article><span>${isEnglish ? "RANKING QUALITY" : "위험순위 품질 지표"}</span><strong>${num(test.pr_auc_average_precision)}</strong><small>PR-AUC · ${isEnglish ? "higher is better" : "높을수록 불량 순위화가 좋음"}</small></article><article><span>${isEnglish ? "FAILURES CAPTURED" : "먼저 찾은 불량"}</span><strong>${ten.captured_fail}<em> / ${ten.total_fail}</em></strong><small>${isEnglish ? `${ten.inspection_count} reviews captured ${pct(ten.fail_capture_rate)} of holdout failures` : `${ten.inspection_count}건 점검으로 전체 불량의 ${pct(ten.fail_capture_rate)} 포착`}</small></article><article><span>${isEnglish ? "LIFT OVER PREVALENCE" : "전체 불량률 대비 농축도"}</span><strong>${num(ten.lift, 2)}<em>${isEnglish ? "×" : "배"}</em></strong><small>${isEnglish ? "review-set failure rate ÷ holdout failure rate" : "점검 범위 내 불량률 ÷ 검증구간 전체 불량률"}</small></article></div><div class="caution"><span>${isEnglish ? "READ THIS" : "꼭 확인하세요"}</span><p>${isEnglish ? "This does not prove factory impact or physical root cause. It tests a review-prioritisation approach on public data." : "실제 공장 성과나 불량 원인을 입증한 결과가 아닙니다. 공개 데이터에서 우선점검 방식의 가능성을 시험한 결과입니다."}</p></div></div></section>
    <section class="advanced-evidence"><div class="section-heading"><div><p class="kicker">PHASE 1 / 고급 검증</p><h2>점추정치보다<br>변동성과 불확실성을 봅니다.</h2></div><p>후기 392건은 같지만, Phase1은 앞 940건 학습·뒤 235건 보정으로 V1의 1,175건 전체 학습과 다릅니다. 비용은 실제 금액이 아니라 시나리오 가정값입니다.</p></div><div class="evidence-cards"><article><span>확률 보정 · ECE</span><strong>${state.phase1.ece.before.toFixed(3)} <i>→</i> ${state.phase1.ece.after.toFixed(3)}</strong><p>예측 확률과 실제 결과의 차이가 감소했습니다. 단, ${state.phase1.ece.bins}개 구간 중 표본이 존재한 구간은 ${state.phase1.ece.populated_bins}개입니다.</p></article><article><span>비용 시나리오 내 최저</span><strong>상위 ${pct(state.phase1.best_cost.k_fraction)}</strong><p>${num(state.phase1.best_cost.total_cost, 0)}점 · 무점검 ${num(state.phase1.best_cost.no_review_cost, 0)}점 대비 ${num(state.phase1.best_cost.reduction, 0)}점 감소</p></article><article><span>${state.phase1.walk_forward.folds}구간 Walk-forward PR-AUC</span><strong>${state.phase1.walk_forward.min.toFixed(3)}–${state.phase1.walk_forward.max.toFixed(3)}</strong><p>구간별 변동이 커서 단일 홀드아웃 수치를 대표 성능으로 볼 수 없습니다.</p></article><article><span>RF vs Logistic · 학습 CV paired</span><strong>Δ ${state.phase1.paired_model_comparison.mean_difference.toFixed(4)} · p=${state.phase1.paired_model_comparison.two_sided_exact_sign_flip_p.toFixed(4)}</strong><p>학습 CV에서 RF가 ${state.phase1.paired_model_comparison.paired_repeats}개 repeat 중 ${state.phase1.paired_model_comparison.challenger_repeat_wins}개에서 높았지만, 5% 기준 통계적 유의성은 확인되지 않았습니다. 후기 평가의 RF 우위를 뜻하지 않습니다. 25개 중첩 fold를 독립 표본으로 세지 않았습니다.</p></article></div><div class="uncertainty-note"><b>${Math.round(state.phase1.top10_capture.confidence * 100)}% 부트스트랩 구간</b><p>상위 10% 불량 포착률은 ${pct(state.phase1.top10_capture.low)}–${pct(state.phase1.top10_capture.high)}로 넓습니다(${state.phase1.top10_capture.bootstrap_replicates.toLocaleString()}회). 희소 불량 표본이 작아 실제 현장 성능으로 확대 해석할 수 없습니다.</p></div></section>
    ...
    <section class="decision-section"><div class="section-heading"><div><p class="kicker">HUMAN-IN-THE-LOOP · SMART FACTORY FIT / 04–05</p><h2>현장 요구에서 점검 큐로,<br>판단과 개선은 사람에게.</h2></div><p>실제 도입 전 현장의 문제와 점검 가능 건수를 먼저 확인한 뒤 MES·FDC·검사 이력을 FabGuard의 검토 순서와 연결합니다. AI는 순서만 제안하고, 엔지니어가 맥락을 확인해 조치와 결과를 기록합니다.</p></div><div class="decision-flow"><article><b>01</b><i>FIELD NEED</i><h3>현장 요구 정의</h3><p>어떤 판단을 도울지, 하루에 몇 건을 실제로 확인할 수 있는지 먼저 정합니다.</p></article><article><b>02</b><i>DATA & PRIORITY</i><h3>근거로 순서 제안</h3><p>생산 이력과 연속 점수를 점검 가능 건수에 맞춰 정렬합니다.</p></article><article><b>03</b><i>HUMAN AUTHORITY</i><h3>엔지니어 판단</h3><p>4M·변동점·품질 이력을 대조하고 재검사·설비점검 여부를 결정합니다.</p></article><article><b>04</b><i>FEEDBACK & AUDIT</i><h3>결과와 피드백 기록</h3><p>조치·최종 품질·사람의 override를 다음 운영규칙을 검토할 근거로 남깁니다.</p></article></div><div class="design-boundary"><span>DESIGN BOUNDARY</span><p>중요 익명변수는 원인이 아닌 점검 후보입니다. SECOM만으로 4M 범주, 실제 센서, 수율 개선 또는 비용 절감 효과를 규명하지 않습니다.</p></div><div class="implementation-note"><b>CURRENT BOUNDARY</b><p>V1은 공개 SECOM 데이터의 오프라인 위험순위화와 Top-K 점검 큐만 구현했습니다. 현장 요구 확인, 실시간 MES/FDC 연동, 생산 제어와 피드백 저장은 목표 구조이며 구현 완료 기능이 아닙니다.</p></div></section>`;

  const expertSelectors = ".advanced-evidence,.pipeline-section,.budget-section,.decision-section";
...
function risksView() {
  ...
  app.innerHTML = `<section class="page-hero"><div><p class="kicker">우선점검 목록</p><h1>먼저 확인할<br><span>생산 건 50개</span></h1><p class="lead">AI가 위험 신호가 큰 순서로 정렬한 목록입니다. 위험점수는 불량 확정값이 아니며, 행을 선택하면 변수 근거의 범위와 해석 한계를 볼 수 있습니다. RF 전체 중요 변수는 모든 건에 동일하며 해당 건의 개별 위험 원인이 아닙니다.</p></div><div class="queue-stat"><span>점검 후보</span><strong>${state.risks.length}</strong><small>위험도 순으로 정렬</small></div></section><section class="table-shell"><div class="panel-head"><span>시간순 검증구간 · 위험도 상위 목록</span><span class="live"><i></i> 데이터 준비됨</span></div><div class="scroll-hint">← 좌우로 밀어 전체 열 보기 →</div><div class="table-wrap"><table><thead><tr><th>순위</th><th>생산 건 ID</th><th>측정 시각</th><th>위험점수</th><th>실제 결과</th><th>근거 범위</th></tr></thead><tbody>${state.risks.map(row => `<tr data-id="${esc(row.sample_id)}" tabindex="0" role="link" aria-label="${esc(row.sample_id)} 상세 점검 결과 열기"><td><b>#${String(row.rank).padStart(2, "0")}</b></td><td>${esc(row.sample_id)}</td><td><span class="timestamp-full">${esc(row.timestamp)}</span><span class="timestamp-short">${esc(shortTimestamp(row.timestamp))}</span></td><td class="risk"><span>${pct(row.risk_score)}</span></td><td><i class="label-dot ${row.label === 1 ? "fail" : "pass"}"></i>${row.label === 1 ? "불량" : "정상"}</td><td>${esc(explanationScope(row.evidence_scope).label)}</td></tr>`).join("")}</tbody></table></div></section>`;
...
function detailView(id) {
  ...
  const features = scope.showFeatures ? String(row.suggested_features).split(";") : [];
  app.innerHTML = `<section class="page-hero detail-hero"><div><p class="kicker">우선순위 ${String(row.rank).padStart(2, "0")}</p><h1>${esc(row.sample_id)}</h1><p class="lead">${esc(row.timestamp)} · 시간순 검증구간의 생산 건</p></div><a class="text-link" href="#risks">← 우선점검 목록</a></section><section class="detail-grid"><article><span>AI 위험점수</span><strong>${pct(row.risk_score)}</strong></article><article><span>고정 기준 예측</span><strong>${row.prediction === 1 ? "불량" : "정상"}</strong></article><article><span>데이터의 실제 결과</span><strong>${row.label === 1 ? "불량" : "정상"}</strong></article></section><section class="evidence-panel"><div><p class="kicker">${esc(scope.label)}</p><h2>${esc(scope.title)}</h2><p>${esc(scope.description)}</p></div><div class="features">${features.map((feature, index) => `<span><b>0${index + 1}</b>${esc(feature)}</span>`).join("")}</div></section><section class="review-prompt"><span>엔지니어 확인</span><p><b>다음 단계:</b> 익명 변수의 실제 센서·공정 매핑 → 4M 및 변동점 기록 → 품질 이력 대조 → 재검사·설비점검 여부를 사람이 결정합니다.</p></section><div class="caution full"><span>해석할 때 주의</span><p>${esc(row.limitation)}</p></div>`;
}

function limitationsView() {
  app.innerHTML = `<section class="page-hero"><div><p class="kicker">${isEnglish ? "VALIDATION AND LIMITS" : "검증과 한계"}</p><h1>${isEnglish ? "How it was tested,<br><span>and what it does not claim</span>" : "어떻게 검증했고,<br><span>무엇을 말하지 않는가</span>"}</h1><p class="lead">${isEnglish ? "The validation design and claim boundaries are public so the result cannot be presented more strongly than the evidence allows." : "결과를 과장하지 않기 위해 검증 방식과 주장하지 않는 범위를 누구나 확인할 수 있게 공개합니다."}</p></div><div class="validation-seal"><span>V1</span><small>${isEnglish ? "PROVISIONAL<br>INDEPENDENT VALIDATION NEEDED" : "잠정 모델<br>독립 검증 필요"}</small></div></section>
    <section class="validation-grid"><article><b>01</b><p class="kicker">VALIDATION DESIGN</p><h2>${isEnglish ? "Temporal validation" : "시간순 검증"}</h2><p>${isEnglish ? "Model selection used 5×5 repeated stratified cross-validation inside the training period. The last 25% was kept as a later temporal holdout, and preprocessing was fitted inside each training fold." : "학습 데이터 내부 5×5 반복 층화 교차검증과 마지막 25% 시간순 홀드아웃을 분리했습니다. 전처리는 각 학습 폴드에만 적합했습니다."}</p></article><article><b>02</b><p class="kicker">MODEL SELECTION</p><h2>${isEnglish ? "Selection rule" : "선택 기준"}</h2><p>${isEnglish ? "Random Forest had the highest training-CV average precision. Fail recall was zero at the untuned 0.5 threshold, so the repository discloses that failure and separately evaluates continuous-score Top-K ranking rather than claiming automated classification." : "Random Forest는 학습 구간 CV의 PR-AUC가 가장 높아 선택했습니다. 0.5 임계값의 Fail recall은 0이었지만, 목표가 자동 판정이 아닌 위험순위화이므로 연속 위험점수의 Top-k 성능을 별도로 평가했습니다."}</p></article><article><b>03</b><p class="kicker">RESULT STATUS</p><h2>${isEnglish ? "Provisional result" : "잠정 결과"}</h2><p>${isEnglish ? "The holdout was exposed during an engineering smoke run. Anonymous variables do not establish physical causality or yield impact; confirmed performance requires a new independent dataset." : "개발 스모크 과정에서 홀드아웃이 먼저 노출됐습니다. 익명 변수의 원인성이나 실제 수율 개선을 주장하지 않으며, 확정 성능에는 독립 데이터 재검증이 필요합니다."}</p></article></section>
    ...
    <section class="boundary-quote"><span>THE HONEST RESULT</span><blockquote>${isEnglish ? `“At the 0.5 threshold, the model classified no Fail records.<br>That failure redirected the project from automated classification to <em>review ranking</em>.”` : `“0.5 임계값에서는 Fail을 분류하지 못했습니다.<br>그래서 자동 판정이 아닌 <em>위험순위화</em>에 집중했습니다.”`}</blockquote><p>${isEnglish ? "The failed performance is disclosed and reframed as decision support under a limited inspection budget." : "실패한 성능을 감추지 않고, 제한된 점검 예산에서 활용 가능한 의사결정 근거로 재정의했습니다."}</p></section>
...
function syncNavigation(hash) {
  const activeRoute = hash.startsWith("detail/") ? "risks" : (hash === "result" || hash === "process" || hash === "answer-overview") ? "summary" : hash;
...
function finishRoute(hash) {
  ...
  window.setTimeout(() => {
    const target = hash === "result" ? document.querySelector(".story-section") : hash === "answer-overview" ? document.querySelector("#answer-overview") : hash === "global" ? document.querySelector(".global-section") : hash === "process" ? document.querySelector("#process") : null;
