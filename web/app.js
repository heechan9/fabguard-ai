  ...
  app.innerHTML = `
    ...
    <section class="global-section" id="global" aria-labelledby="global-title">
      ...
      <div class="country-grid" aria-label="국가별 데이터 검증 상태">
        ...
        <article class="country-card next"><div class="country-top">${flagImage("fr", isEnglish ? "France" : "프랑스")}<b>${isEnglish ? "FRANCE" : "프랑스"}</b><em>${isEnglish ? "PARTIAL E2E" : "부분 E2E"}</em></div><h3>RTE · Enedis · Météo-France</h3><p>${isEnglish ? "National generation revisions · distribution solar · observed Paris weather" : "국가 발전량 수정 이력 · 배전망 태양광 · 파리 관측 기상"}</p><dl><div><dt>${isEnglish ? "ROLE" : "역할"}</dt><dd><span class="role-pair">${dataRole("estimated", isEnglish ? "Grid estimate" : "전력망 추정")}${dataRole("observed", isEnglish ? "Weather observed" : "기상 관측")}</span></dd></div><div><dt>${isEnglish ? "STATUS" : "현황"}</dt><dd>${isEnglish ? `Enedis ${enedis.input_rows.toLocaleString()} intervals · Météo-France ${meteo.input_rows.toLocaleString()} hours verified · RTE annual rerun pending` : `Enedis ${enedis.input_rows.toLocaleString()}개 간격 · Météo-France ${meteo.input_rows.toLocaleString()}시간 검증 · RTE 연간 재실행 대기`}</dd></div></dl></article>
      </div>
