...
function summaryView() {
  ...
  app.innerHTML = `
    <section class="hero"><div class="hero-copy"><div class="status-chip"><i></i> SEMICONDUCTOR QUALITY SUPPORT · DEMO</div><p class="kicker">FABGUARD AI</p><h1>불량을 확정하기 전에,<br><span>점검 순서부터.</span></h1><p class="hero-ko">반도체 생산 데이터에서<br><strong>먼저 살펴볼 생산 건을 찾습니다.</strong></p><p class="lead">FabGuard는 590개 측정값을 분석해 위험도가 높은 생산 건을 앞에 배치합니다. AI가 합격·불합격을 결정하는 것이 아니라, 엔지니어가 어디부터 확인할지 돕는 서비스입니다.</p><div class="hero-actions"><a class="button" href="#risks">우선점검 목록 보기 <span>→</span></a><a class="text-link" href="#limitations">AI가 하지 않는 일 보기</a></div></div>${waferVisual()}</section>
    <section class="plain-guide" aria-label="FabGuard 작동 방식"><div><span>01 · 데이터 입력</span><strong>생산 과정의 측정값</strong><p>공개 데이터에 포함된 1,567건의 생산 기록과 590개 익명 변수를 사용합니다.</p></div><div><span>02 · AI 분석</span><strong>위험도가 높은 순서로 정렬</strong><p>모든 생산 건을 판정하지 않고, 제한된 점검 시간을 어디에 먼저 쓸지 제안합니다.</p></div><div><span>03 · 사람의 판단</span><strong>엔지니어가 확인하고 결정</strong><p>실제 센서·설비·공정 이력을 대조한 뒤 재검사와 설비점검 여부를 결정합니다.</p></div></section>
    <section class="data-strip" aria-label="프로젝트 데이터 요약"><div><span>01</span><strong>1,567</strong><small>분석한 생산 기록</small></div><div><span>02</span><strong>590</strong><small>익명 측정변수</small></div><div><span>03</span><strong>24</strong><small>검증구간 실제 불량</small></div><div><span>04</span><strong>50</strong><small>우선점검 생산 건</small></div></section>
