...
function summaryView() {
  ...
  app.innerHTML = `
    ...
    <section class="advanced-evidence"><div class="section-heading"><div><p class="kicker">PHASE 1 / 재현 실험</p><h2>좋아진 부분과<br>불확실성을 함께 봅니다.</h2></div><p>같은 시간순 테스트 구간을 유지한 추가 실험입니다. 비용은 실제 금액이 아니라 점검 전략을 비교하기 위한 가정값입니다.</p></div><div class="evidence-cards"><article><span>확률 보정</span><strong>${state.phase1.ece.before.toFixed(3)} <i>→</i> ${state.phase1.ece.after.toFixed(3)}</strong><p>예측 확률과 실제 결과의 차이가 감소했습니다.</p></article><article><span>비용 가정 내 최저</span><strong>상위 ${pct(state.phase1.best_cost.k_fraction)}</strong><p>${state.phase1.best_cost.total_cost}점 · 무점검 ${state.phase1.best_cost.no_review_cost}점 대비 ${state.phase1.best_cost.reduction}점 감소</p></article><article><span>시간구간별 PR-AUC</span><strong>${state.phase1.walk_forward.min.toFixed(3)}–${state.phase1.walk_forward.max.toFixed(3)}</strong><p>구간마다 차이가 커서 지속적인 재검증이 필요합니다.</p></article></div><div class="uncertainty-note"><b>왜 잠정 결과인가요?</b><p>상위 10% 불량 포착률의 95% 구간은 ${pct(state.phase1.top10_capture.low)}–${pct(state.phase1.top10_capture.high)}로 넓습니다. 희소 불량 구간의 표본도 작아 실제 현장 성능으로 확대 해석할 수 없습니다.</p></div></section>
