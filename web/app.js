...
function risksView() {
  ...
  app.innerHTML = `<section class="page-hero"><div><p class="kicker">PRIORITY QUEUE</p><h1>Risk-ranked<br><span>production runs.</span></h1><p class="lead">모델 위험도 상위 50건입니다. 행을 선택하면 우선 확인할 익명 측정변수와 해석 한계를 확인할 수 있습니다.</p></div><div class="queue-stat"><span>QUEUE SIZE</span><strong>${state.risks.length}</strong><small>RANKED INSTANCES</small></div></section><section class="table-shell"><div class="panel-head"><span>PRIORITY QUEUE / TEMPORAL HOLDOUT</span><span class="live"><i></i> DATA READY</span></div><div class="table-wrap"><table><thead><tr><th>RANK</th><th>PRODUCTION RUN</th><th>TIMESTAMP</th><th>RISK SCORE</th><th>ACTUAL</th><th>EVIDENCE</th></tr></thead><tbody>${state.risks.map(row => `<tr data-id="${esc(row.sample_id)}"><td><b>#${String(row.rank).padStart(2, "0")}</b></td><td>${esc(row.sample_id)}</td><td>${esc(row.timestamp)}</td><td class="risk"><span>${pct(row.risk_score)}</span></td><td><i class="label-dot ${row.label === 1 ? "fail" : "pass"}"></i>${row.label === 1 ? "FAIL" : "PASS"}</td><td>${esc(row.evidence_scope)}</td></tr>`).join("")}</tbody></table></div></section>`;
...
function limitationsView() {
  app.innerHTML = `<section class="page-hero"><div><p class="kicker">VALIDATION & BOUNDARIES</p><h1>Evidence first.<br><span>Claims second.</span></h1><p class="lead">결과를 과장하지 않기 위해 검증 방식과 주장하지 않는 범위를 명확하게 공개합니다.</p></div><div class="validation-seal"><span>V2</span><small>RESEARCH<br>PROTOTYPE</small></div></section><section class="validation-grid"><article><b>01</b><p class="kicker">VALIDATION DESIGN</p><h2>시간순 검증</h2><p>학습 데이터 내부 5×5 반복 층화 교차검증과 마지막 25% 시간순 홀드아웃을 분리했습니다. 전처리는 각 학습 폴드에만 적합했습니다.</p></article><article><b>02</b><p class="kicker">NON-CLAIMS</p><h2>주장하지 않는 것</h2><p>익명 변수의 물리적 의미나 불량 원인, 실제 수율 개선, SPC·FDC·APC 시스템 구축 효과를 주장하지 않습니다.</p></article><article><b>03</b><p class="kicker">RESULT STATUS</p><h2>잠정 결과</h2><p>개발 스모크 과정에서 홀드아웃이 먼저 노출됐습니다. 확정 성능을 주장하려면 독립 데이터 재검증이 필요합니다.</p></article></section><section class="boundary-quote"><span>THE HONEST RESULT</span><blockquote>“0.5 임계값에서는 Fail을 분류하지 못했습니다.<br>그래서 자동 판정이 아닌 <em>위험순위화</em>에 집중했습니다.”</blockquote><p>실패한 성능을 감추지 않고, 제한된 점검 예산에서 활용 가능한 의사결정 근거로 재정의했습니다.</p></section>`;
}
