// Synthetic demonstration only. Keep parity with fabguard.spc.individuals_report.
export function individuals(values, count = 10) {
  ...
  const blocked = baseline.some(v=>v<lcl || v>ucl);
  return {center,lcl,ucl, statuses:values.slice(count).map(v=>v===null?'unknown':blocked?'baseline_review_required':v<lcl?'below_limit':v>ucl?'above_limit':'within_limits')};
}
