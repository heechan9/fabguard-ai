...
def normalize_eco2mix_records(
    ...
    *,
    ...
    """Normalize one complete bounded page-set from the official records API."""
    ...
    solar = pd.to_numeric(frame["solaire"], errors="coerce")
    if solar.isna().any() or not np.isfinite(solar.to_numpy(dtype=float)).all():
        raise RTEEco2MixContractError("solaire contains missing, non-numeric, or infinite values")
    if (solar < 0).any():
    ...
    normalized = pd.DataFrame({
        ...
        "revision_status": statuses,
        "source_timestamp": frame["date_heure"].astype(str),
