...
| 국가 | 후보 데이터 | 검토 목적 | 현재 판정 |
|---|---|---|---|
| 🇫🇮 핀란드 | **FMI 고위도 PV 데이터** | 적설·저온 환경 스트레스 테스트 | 조건부 1순위 후보·접근/라이선스 확인 필요 |
| 🇨🇦 캐나다 | **OSOTF** | 적설 환경 실제 발전량 검증 | 현재 다운로드·데이터 라이선스 미확인 |
| 🇯🇵 일본 | **규슈전력 태양광 발전실적** | 출력제어를 포함한 계통운영 분석 | 현 PV 데이터계약과 다른 후속 연구로 보류 |
| 🇹🇼 대만 | **WM-811K 웨이퍼맵** | 반도체 결함 패턴 분석 가능성 검토 | 이미지 데이터로 현재 시계열 연구와 달라 제외 |

일본·대만은 국가 수를 늘리기 위한 항목이 아닙니다. 별도의 연구질문이 성립하고 출처·라이선스·스키마 자격을 통과할 때만 새 프로젝트로 재검토합니다.
