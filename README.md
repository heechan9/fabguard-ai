...
| 🇮🇱 이스라엘 | **data.gov.il PV 시계열 후보 탐색** | 고온·사막먼지에 따른 PV 손실 스트레스 | 실제 발전량 시계열·라이선스·단위·시간대 확인 전 조사 후보 |
| 🇸🇦 사우디아라비아 | **Southampton ePrints 제다 가정용 PV 실측 후보** | 고온·사막먼지에 따른 PV 손실 스트레스 | 원 데이터 기록·DOI·재사용 조건·스키마 확인 전 조사 후보 |

중동 후보는 프랑스 3종 사전감사보다 뒤에 둡니다. UAE Shams Dubai 설치 건수와 카타르 유료 통계는 발전량 시계열이 아니므로 제외하며, 군사시설 에너지 데이터는 보안·재사용권 문제로 검토하지 않습니다.
